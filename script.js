        ...
        html += `
            <a href="${item.link}" target="_blank" id="card-${item.id}" class="${cardClass}" data-id="${item.id}" style="text-decoration: none; -webkit-tap-highlight-color: transparent;">
                ...
                <div class="card-content">
                    <span style="font-weight: 700; font-size: 16px; color: #784212; margin-bottom: 6px;">${item.id}</span>
                    <p style="font-size: 13px; color: #333; margin: 0; line-height: 1.4; display: -webkit-box; -webkit-line-clamp: 4; -webkit-box-orient: vertical; overflow: hidden;">
                        ${item.title}
                    </p>
                </div>
            </a>
...
// --- 3. HÀM TÌM KIẾM VÀ CUỘN (KHI KHÁCH NHẬP MÃ VÀO Ô INPUT) ---
function searchAndScroll() {
    ...
    if (targetCard) {
        // Đánh dấu là đang tự động cuộn để không bị sự kiện scroll người dùng bắt nhầm
        isAutoScrolling = true;

        // Làm nổi bật và làm mờ các ô khác
        ...
        // Sau khi cuộn xong (khoảng 600ms), mở khóa lại cờ cuộn
        setTimeout(() => {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Lắng nghe sự kiện cuộn trang từ người dùng thực tế
    window.addEventListener('scroll', () => {
        // Nếu KHÔNG phải do code tự động cuộn, VÀ người dùng thực sự cuộn trang xuống
        if (!isAutoScrolling && (window.scrollY > 15 || document.documentElement.scrollTop > 15)) {
            resetSearchState();
        }
    });
});